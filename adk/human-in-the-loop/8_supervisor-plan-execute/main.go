...
func main() {
	...
	sv, err := buildProjectManagerSupervisor(ctx)
	if err != nil {
		log.Fatalf("build project manager supervisor failed: %v", err)
	}
	...
	query := `Set up a new project called "Customer Portal" for the engineering department. 
This should include analyzing requirements, creating a design, allocating a budget of $150,000 from engineering, 
and assigning a team of Alice, Bob, and Charlie starting from 2025-02-01.`
	...
	for {
		...
		for {
			...
			fmt.Print("Approve this budget allocation? (Y/N): ")
			scanner.Scan()
			fmt.Println()
			nInput := scanner.Text()
